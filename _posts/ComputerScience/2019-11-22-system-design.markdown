...
### 5 消息队列
整体架构：一般分为 producer，broker，consumer 三者
...
可靠性
- 生产者：发送消息前，开启事务，然后发送消息，如果发送过程中出现什么异常事务就会回滚；如果发送成功则提交事务。
- 消息队列：开启持久化磁盘的配置
- 消费者：手动确认消息

RPC（Remote Procedure Call Protocol）远程过程调用协议：客户端在不知道调用细节的情况下，调用存在于远程计算上的某个过程或函数，就像调用本地应用程序中的一样。
+ TCP 协议是 RPC 的基石
+ I/O 模型
...
+ Schema 和序列化：当 I/O 完成后，序列化和反序列化，是进行对象到二进制数据的转换，程序是可以理解对象的。常见的方式是 Protocol Buffers（简称 PB）
+ 