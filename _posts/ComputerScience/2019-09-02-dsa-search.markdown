...
#### 2.2 红黑树
红黑树是每个节点都带有颜色属性的二叉查找树。典型的用途是实现关联数组。将红黑树的红链接画平即为 2-3 树。
...
3. 所有的叶子结点和相连的节点使用链表相连；
4. 所有关键字都在叶子结点出现；

B+ 树的优点在于：
- 由于 B+ 树在内部节点上不好含数据信息，因此在内存页中能够存放更多的key。 数据存放的更加紧密，具有更好的空间局部性。因此访问叶子几点上关联的数据也具有更好的缓存命中率。
