...
```Java
Integer x = 1;    // 装箱，自动将基本数据类型转换为包装器类型 调用了 Integer.valueOf(1)
int y = x;        // 拆箱，自动将包装器类型转换为基本数据类型 调用了 X.intValue()
...
- Map
  ...
  - `ConcurrentHashMap`：和 HashMap 实现上类似，采用了分段锁（Segment），每个分段锁维护着几个桶（HashEntry），多个线程可以同时访问不同分段锁上的桶，从而使其并发度更高。
...
- 轻量级锁：是相对于传统的重量级锁而言，它使用 CAS（比较并交换，Compare-and-Swap）操作来避免重量级锁使用互斥量的开销。
...
- Java 内存模型定义了 8 个操作来完成主内存和工作内存的交互操作：read、load、use、assign、store、write、lock、unlock
